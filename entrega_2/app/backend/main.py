...
# Crear campo product_name legible
...
def recommend_products(customer_id: str, top_k: int = 5, exclude_product: str = None):
    next_week = pd.Timestamp.now().normalize() + pd.Timedelta(days=7)
    semana = next_week.to_period("W").start_time
    ...
    # Opción: excluir producto ya predicho
    if exclude_product:
        df = df[df["product_id"] != int(exclude_product)]
